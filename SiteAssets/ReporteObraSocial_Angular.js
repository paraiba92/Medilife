    ...
    myAngApp.controller('spResultado', function ($scope, $location, $http, $anchorScroll, $timeout) {
        ...
        $scope.ejecutarWS = function () {
            ...
            $timeout(function () {
                ...
                var elementos = SPD_GetListItems_CamlQuery(_spPageContextInfo.webAbsoluteUrl, MD_OS, viewXml).results
        ...
        function procesarJSON(elementos) {
           ...
           for(rec=0; rec < elementos.length; rec++) {
                var rta = true;
                if(filterNombre != "") {
                    if(elementos[rec].Nombre.toUpperCase().indexOf(filterNombre.toUpperCase()) == -1){
                           rta = false;
                    }
                }
                if(filterEmail != "") {
